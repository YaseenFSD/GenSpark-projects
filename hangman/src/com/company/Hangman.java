...
public class Hangman {
    ...
    public void start(Scanner sc) {
        ...
        char[] currentGuess = getFillerArray();
        while (!gameDone){
            printHangman();
            printLetters(currentGuess);
            char input = sc.next().charAt(0);
            if (hasLetter(input)) {
                updateGuess(currentGuess, input);
            }else {
...
//                    historyLetters.clear();
    ...
    void updateGuess(char[] currentGuess, char correctChar){
    ...
    void printLetters(char[] guessedLetters){
    ...
    char[] getFillerArray() {
        char[] fillers = new char[answer.length()];
